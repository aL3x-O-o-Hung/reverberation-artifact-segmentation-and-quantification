...
"""
The previous hierarchical probabilistic U-Net, which is the first network in our paper
"""
...
def load_data(input_dir1,input_dir2):
    ...
    while os.path.exists(input_dir1+str(i)+'.png'):
        ...
        needle=cv2.cvtColor(needle,cv2.COLOR_BGR2GRAY)/255.0
        artifact=cv2.cvtColor(artifact,cv2.COLOR_BGR2GRAY)/255.0
        needle_var=cv2.cvtColor(needle_var,cv2.COLOR_BGR2GRAY)/255.0
        artifact_var=cv2.cvtColor(artifact_var,cv2.COLOR_BGR2GRAY)/255.0
        needle=np.reshape(needle,(256,256,1))
        artifact=np.reshape(artifact,(256,256,1))
        needle_var=np.reshape(needle_var,(256,256,1))
        artifact_var=np.reshape(artifact_var,(256,256,1))
